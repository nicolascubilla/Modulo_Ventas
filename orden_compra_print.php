...
$pdf->Cell(0, 0, 'REPORTE DE ORDEN DE COMPRA', 'B', 0, 'C', 0);
$pdf->Ln(10);

// Datos de la orden
$orden_id = $_REQUEST['orden_id'];
$cabeceras = consultas::get_datos("SELECT DISTINCT * FROM v_orden_compra_cabecera_detalle WHERE orden_id = $orden_id");
...
if (!empty($cabeceras)) {
    ...
    $pdf->Cell(0, 0, 'Orden N° ' . $orden_id, 0, 1, 'C', 0);
    $pdf->Ln(10);

    $pdf->SetFont('times', '', 10);
// Primera fila de datos
$pdf->Cell(110, 5, 'PRESUPUESTO N°: ' . $cabecera['presupuesto_id'], 0, 0, 'L');
$pdf->Cell(100, 5, 'SUCURSAL: ' . $cabecera['sucursal'], 0, 1, 'L'); // Ancho extendido para "SUCURSAL"

// Segunda fila de datos
$pdf->Cell(110, 5, 'USUARIO: ' . $cabecera['usuario'], 0, 0, 'L');
$pdf->Cell(100, 5, 'PROVEEDOR: ' . $cabecera['proveedor'], 0, 1, 'L'); // Ancho extendido para "PROVEEDOR"

// Tercera fila (Fecha y plazo de entrega)
$pdf->Cell(110, 5, 'FECHA: ' . $cabecera['fecha_orden'], 0, 0, 'L');
$pdf->Cell(100, 5, 'PLAZO DE ENTREGA: ' . $cabecera['plazo_entrega'], 0, 1, 'L'); // Ancho extendido para "PLAZO DE ENTREGA"

// Estado destacado (en línea independiente)
$pdf->Ln(2);
...
$pdf->SetTextColor(0, 0, 0);
    ...
    $detalles = consultas::get_datos("SELECT * FROM v_orden_compra_cabecera_detalle WHERE orden_id = $orden_id ORDER BY nombre_material");
    if (!empty($detalles)) {
        ...
        foreach ($detalles as $detalle) {
            ...
            $pdf->Cell(90, 7, $detalle['nombre_material'], 1, 0, 'L');
    ...
    $pdf->SetFont('times', '', 10);
    // Convierte el total en letras (puedes agregar tu propia función de conversión)
    ...
    $pdf->Cell(0, 5, 'No se encontró la orden de compra.', 0, 1, 'C');
}
