...
.btn-custom:hover {
    background-color: #ffaa00; /* Un amarillo más oscuro al pasar el ratón */
    color: #000;
...
.btn-custom {
    margin-right: 10px;
    margin-top: -35px; /* Mueve el botón hacia arriba */
}
...
<form action="control_calidad_control.php" method="post" class="form-horizontal">
                        ...
                        <div class="row">
                            <div class="col-lg-4">
                                ...
                                <select class="form-control" name="vcontrol_id" id="control" onchange="cargarcontrol(this.value)" required>
                            ...
                            <div class="col-lg-4">
                                ...
                                <div class="input-group">
                                    <input type="text" id="buscar_produccion" class="form-control" placeholder="Ingrese ID" autocomplete="off">
                                    <span class="input-group-btn">
                                        <button type="button" class="btn btn-primary" onclick="buscarproduccion()">Buscar</button>
                                    </span>
                                </div>
                            </div>
                            <div class="col-lg-4">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-6">
                                ...
                                <textarea id="observacion" name="vobservacion" class="form-control" placeholder="Ingrese una observación..."></textarea>
                            </div>
                        </div>
                        <div class="row">
...
<div class="col-lg-6">
        ...
        <?php } ?>
                ...
                <tbody id="detalle_control">
                    <tr>
                        <td colspan="5" class="text-center">Seleccione un pedido para cargar el detalle.</td>
                    </tr>
                </tbody>
                            </table>
                        </div>
...
function cargarcontrol(control_id) {
    ...
    error: function (xhr, status, error) {
        ...
        $("#detalle_control").html('<tr><td colspan="5" class="text-center">Error al cargar el detalle de la producción.</td></tr>');
    }
});

    } else {
        $("#detalle_control").html('<tr><td colspan="5" class="text-center">Debe seleccionar una producción.</td></tr>');
    }
}
...
function buscarproduccion() {
    ...
    if (query) {
        $.ajax({
            ...
            success: function (response) {
                ...
                if (firstOption) {
                    ...
                    $('#detalle_control').html('<tr><td colspan="5" class="text-center">No se encontró ningún pedido con el ID ingresado.</td></tr>');
                }
            },
            error: function () {
        ...
        $('#detalle_control').html('<tr><td colspan="5" class="text-center">Ingrese un ID de la producción.</td></tr>');
    }
}
...
function toggleCantidad(selectElement) {
    ...
    if (selectElement.value == "9") {  // Estado "Rechazado"
        cantidadRechazada.disabled = false;
        cantidadCritica.disabled = true;
        cantidadCritica.value = "";  
    } else if (selectElement.value == "10") {  // Estado "Crítico"
        cantidadRechazada.disabled = true;
        cantidadCritica.disabled = false;
